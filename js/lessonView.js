...
function hasMissingSemicolon(code) {
    ...
    for (let raw of lines) {
        ...
        // blokkok nyitása/zárása
        if (line.endsWith("{") || line.endsWith("}")) continue;
        // objektum/tömb elemek
        if (line.endsWith(",")) continue;
        // objektum tulajdonság: kulcs:
        if (/^[a-zA-Z_$][\w$]*\s*:/.test(line)) continue;
        // vezérlési szerkezetek fejrésze
        if (
        ...
        // ha egyik fenti kategóriába sem esik, akkor utasításnek tekintjük,
        // és elvárjuk a pontosvesszőt
        if (!line.endsWith(";")) {
            return true;
        }
    }
...
function runTask(button) {
    ...
    // mindig töröljük a demó gomb onclick-jét, mielőtt bármit csinálnánk
    const demoBtn = document.getElementById("gomb");
    if (demoBtn) {
        demoBtn.onclick = null;
    }

    // 0–99-es tartomány ellenőrzés, ha kérve van
    if (range === "0-99") {
        const numRegex = /-?\d+/g;
        let match;
    ...
    // hiányzó pontosvessző ellenőrzés
    ...
    // STRICT módban először csak összehasonlítjuk a kódot – NEM futtatjuk
    ...
    try {
        ...
        const logText = logs.length
            ...
            : "<em>(A program nem írt ki semmit – ha a kód a weboldal elemeit módosítja, a változást feljebb látod.)</em>";
